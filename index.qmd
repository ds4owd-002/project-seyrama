...
Functionality of sanitation infrastructure
...
processed_data |> 
  mutate(hcf_waste_segregated_clean = if_else(
    hcf_waste_segregated == "Yes",
    "Yes", "No"))
...
processed_data |> 
  group_by(hcf_level, hcf_waste_mgt_responsible) |> 
  summarise(
    number_facilities = n(),
    prop_segregated = round(mean(hcf_waste_segregated == "Yes", na.rm = .groups = "drop")) |> 
      kable(caption = "Comparison of health care waste management practices by presence of a waste management officer")
  )
  count(hcf_waste_segregated, hcf_waste_SOP, hcf_waste_treatment_general) |> 
  mutate(percent = round(n/sum(n) * 100, 0)) |> 
  kable(caption = "Comparison of health care waste management practices by presence of a waste management officer")
```
